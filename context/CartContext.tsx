...
interface CartContextType {
  ...
  clearCart: () => void;
  syncCartWithCustomer: () => Promise<void>;
  updateBuyerIdentity: (buyerInfo: {
    email: string;
    phone: string;
    firstName: string;
    lastName: string;
...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateBuyerIdentity = async (buyerInfo: {
    email: string;
    phone: string;
    firstName: string;
    lastName: string;
    ...
    setIsLoading(true);
    try {
      const updatedCart = await updateCartBuyerIdentity(currentCartId, buyerInfo);
      if (updatedCart) {
        setCheckoutUrl(updatedCart.checkoutUrl);
      }
    } catch (e) {
